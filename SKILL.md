...
- **规则 31 · CDN/WAF 数据中心出口拦截识别**：同一测试环境昨日可通、今日全站拦截（静态资源/robots.txt 同拦），拦截页为"纯拒绝"形态（自定义状态码 + 标准文案，无挑战无滑块）时，优先怀疑 **IP 段被静态规则拦截**（数据中心/IDC 来源常见），而非站点故障。验证三步：①多 UA 复测（排除 UA 规则）②真浏览器复测（排除行为检测）③有条件时对照住宅网络视角。区分分层：自定义状态码（如 567）+ 拦截页 = WAF 层；522/504 + 超时 = 回源层；404/401 快速返回 = 源站进程存活。**处置**：向目标方提供出口 IP + 拦截响应头中的 Request ID 请求加白，勿反复重试（可能加重风控画像）。
- **规则 32 · 测试前置可达性检查**：对已知站点的每次测试会话开始前，先对主域 + API 域 + robots.txt 做 3 点位可达性探测，并记录当日出口 IP 与归属（数据中心/住宅）；出口 IP 变化或命中拦截时，先解决通道问题再排任务——**把"通道健康"从任务内第一步提为任务前置条件**，避免把 WAF 拦截误判为目标故障，或把时间耗在注定失败的请求上。配套：拦截状态下的所有结论（"全站不可用""防护升级"）必须标注"本环境视角"，不可外推为全局状态。
